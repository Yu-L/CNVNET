...
Created by Yu Liu on 2019/9/27 9:17 AM.
"""
...
sys.path.append(os.path.join(packages_dir,".."))
...
def get_feat_seg(seg_s, seg_end):
    ...
    if valide_len == 0:
        # no reads cover
        lock.release()
        return seg_s, seg_end, seg_len, 1, None, cov_rate
    elif valide_len < win_len.value * min_r.value:
        # the number of reads coverage is not enough
        lock.release()
        return seg_s, seg_end, seg_len, 2, None, cov_rate
    else:
        ...
        lock.release()
        return seg_s, seg_end, seg_len, 3, i_feat_mat, cov_rate
...
def main(args):
    ...
    online_in_root_dir = args.online_in_root_dir
    ...
    if not os.path.isdir(online_out_sample_subdir):
        os.makedirs(online_out_sample_subdir)
    online_out_sample_data_dir = os.path.join(online_out_sample_subdir, 'data')
    if not os.path.isdir(online_out_sample_data_dir):
        os.makedirs(online_out_sample_data_dir)

    # this was generated by STEP 20
    in_online_sample_data_fn = os.path.join(online_in_root_dir,sample_id, 'data',
    ...
    gap_out = np.array([[i_gap_start, i_gap_start + i_gap_lens, i_gap_lens, 0, 0]
    ...
    if len(gap_out) > 0:
        ...
        with h5py.File(out_feat_segs_fn + '_gap.h5', 'w') as h5_out:
            h5_out.create_dataset('gap', data=gap_out, maxshape=(None, 5), compression="gzip", compression_opts=9,
    ...
    with multiprocessing.Pool(n_proc, initializer=mp_init, initargs=(locker,)) as p:
        ...
        for i_idx, (i_seg_s, i_seg_end, i_seg_len, i_indicator, i_feat_mat, i_cov_rate) in enumerate(results):
            if i_idx % n_chk_size == 0:
                logger.info('finished at {}, {} of {}, type {}, base coverage rate{}'.format(i_seg_s, i_idx, n_seg_lens_, i_indicator, i_cov_rate))

            if i_indicator == 3:
                pred_start_p = int(np.minimum(i_seg_s, pred_start_p))
                pred_end_p = int(np.maximum(i_seg_end, pred_end_p))

                predictable_meta_arr.append(np.array([i_seg_s, i_seg_end, i_seg_len, i_indicator, i_cov_rate], dtype=np.int32))
                ...
                if len(predictable_meta_arr) >= saved_len:
                    ...
                    with h5py.File(i_pred_fn, 'w') as pred_h5:
                        logger.info('h5py start writing {} pred results, file index: {}'.format(
                        len(predictable_meta_arr), pred_save_idx))
                ...
                unpredictable_meta_arr.append(np.array([i_seg_s, i_seg_end, i_seg_len, i_indicator, i_cov_rate], dtype=np.int32))
...
if __name__ == "__main__":
    ...
    type=str,
    default='/zfssz2/ST_MCHRI/BIGDATA/PROJECT/NIPT_CNV/f_cnv_out/online',
    help="online whole chromosome features input directory")
