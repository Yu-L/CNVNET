...
Created by Yu Liu on 2019/8/20 2:38 PM.
"""
import os
import numpy as np
import pandas as pd
...
from train_model import CNVDataGenerator, CNVDataGenerator2, CNVDataGenerator3
...
def train2(target_train_h5_fn, target_val_h5_fn, model_out_root_dir,
          n_win_size=5000, n_feat=13, n_class=3,
          epochs=64, batch=1024, learn_rate=0.001, drop=0.5,
          fc_size=64, blocks='4_4_3', l2r=1e-4, temperature=5, lbl_smt_frac=0.1,filters=32, kernel_size=16, strides=1, pool_size=2,
                         pool_stride=2, pw=0, n_gpu=4, n_cpu=20):
    ...
    model_name = 'cnvnet'
    base_model = cnv_net(n_win_size, n_feat, n_class, filters=filters, kernel_size=kernel_size, strides=strides, pool_size=pool_size,
                         pool_stride=pool_stride, drop=drop, blocks=_blocks, fc_size=fc_size,
                         kernel_regular_l2=l2r, temperature=temperature, m_name=model_name)
