...
def online_call_mp_gpu(online_seg_data_root_dir, online_call_out_root_dir, model_in_root_dir,
    ...
    def out_name():
        ...
        return 'b{0}_ep{1}_lr{2:.3f}_dr{3:.1f}_fc{4}_' \
               'blk{5}_win{6}_l2r{7}_temp{8}_lblsmt{9}_pw{10}'.format(
            batch, epochs, learn_rate, drop, fc_size, str_blk, n_win_size, str(l2r),
            temperature, int(lbl_smt_frac) if lbl_smt_frac == 0 else lbl_smt_frac, pw)
